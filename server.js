...
app.post('/api/submit', (req, res) => {
  const { token, orgId, wechat, email, sessionData } = req.body;
  const tokens = loadTokens();
  ...
  const pkg = (t.packageType || '').toLowerCase();
  const isChatGPT = pkg.includes('chatgpt');

  const errors = {};
  if (!isChatGPT) {
    if (!orgId || !UUID_REGEX.test(orgId.trim())) errors.orgId = 'Invalid Organization ID format.';
  } else {
    if (!sessionData || !sessionData.trim()) errors.orgId = 'ChatGPT session data is required.';
  }
  ...
  const orgIdLine = isChatGPT
    ? `Session Data     : [ChatGPT session provided — ${sessionData.trim().length} chars]`
    : `Organization ID  : ${orgId.trim()}`;
  const entry = ['══════════════════════════════════════════════════════',`Submitted At     : ${timestamp}`,`Customer         : ${t.customerName}`,`Package          : ${t.packageType}`,'── Details ────────────────────────────────────────────',orgIdLine,`WeChat           : ${wechat.trim()}`,`Email            : ${email.trim()}`,'══════════════════════════════════════════════════════',''].join('\n');
  fs.appendFileSync(SESSIONS_FILE, entry);
  tokens[token].used = true; tokens[token].submittedAt = timestamp;
  tokens[token].orgId = isChatGPT ? '' : orgId.trim();
  tokens[token].sessionData = isChatGPT ? sessionData.trim() : '';
